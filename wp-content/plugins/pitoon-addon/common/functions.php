...
if (!function_exists('pitoon_get_fa_icons')) :
    ...
    function pitoon_get_fa_icons()
    {
        ...
        if (empty($data)) {
            ...
            $fontAwesome_file =   PITOON_ADDON_PATH . '/assets/vendors/fontawesome/css/all.min.css';
            $template_icon_file = PITOON_ADDON_PATH . '/assets/vendors/pitoon-icons/style.css';
            $content = '';
            ...
            comments_popup_link(
                sprintf(
                    wp_kses(
                        ...
                        __('Leave a Comment<span class="screen-reader-text"> on %s</span>', 'pitoon'),
                        array(
                            'span' => array(
                                'class' => array(),
                            ),
    ...
    function pitoon_entry_footer()
    {
        ...
        if ('post' === get_post_type()) {
            ...
            if ($categories_list) {
                ...
                printf('<span class="blog-details__tags cat-links"><span>' . esc_html__('Posted in: %1$s', 'pitoon') . '</span>', '</span>' . $categories_list); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
            }
            ...
            if ($tags_list) {
                ...
                printf('<span class="blog-details__tags blog-details__tags--tags tag-links"><span>' . esc_html__('Tags: %1$s', 'pitoon') . '</span>', '</span>' . $tags_list); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
            }
        }
    }
endif;
...
if (!function_exists('pitoon_get_template')) :
    function pitoon_get_template($template_name = null)
    {
        $template_path = apply_filters('pitoon-elementor/template-path', 'elementor-templates/');
        $template = locate_template($template_path . $template_name);
        if (!$template) {
            $template = PITOON_ADDON_PATH  . '/elementor-templates/' . $template_name;
        }
...
if (!function_exists('pitoon_get_elementor_option')) :
    function pitoon_get_elementor_option($template_name = null)
    {
        $template_path = apply_filters('pitoon-elementor/template-options', 'elementor-options/');
        $template = locate_template($template_path . $template_name);
        if (!$template) {
            $template = PITOON_ADDON_PATH  . '/elementor-options/' . $template_name;
        }
        if (file_exists($template)) {
            return $template;
        }
...
if (!function_exists('pitoon_get_thumbnail_alt')) :
    function pitoon_get_thumbnail_alt($thumbnail_id)
    {
...
if (!function_exists('pitoon_get_owl_options')) :
    function pitoon_get_owl_options($settings)
    {
...
if (!function_exists('pitoon_get_circle_options')) :
    function pitoon_get_circle_options($arg, $condition = false)
    {
        ...
        if (!empty($condition)) :
            ...
            $arg->start_controls_section(
                'circle_options',
                [
                    'label' => __('Circle Options', 'pitoon-addon'),
        ...
        $arg->add_control(
            ...
            [
                'label' => __('Enable Gradient Color?', 'pitoon-addon'),
        ...
        $gradient_items_color->add_control(
            'gradient_color',
            [
                'label' => esc_html__('Gradient Color', 'pitoon-addon'),
        ...
        $arg->add_control(
            ...
            [
                'label' => __('Gradient Lists', 'pitoon-addon'),
        ...
        $arg->add_control(
            'fill_color',
            [
                'label' => esc_html__('Fill Color', 'pitoon-addon'),
                'type' => \Elementor\Controls_Manager::COLOR,
                'default=' => '36731f',
                'condition' => [
                    'enable_gradient_color!' => ['yes']
        ...
        $arg->add_control(
            'empty_fill_color',
            [
                'label' => esc_html__('Empty Fill Color', 'pitoon-addon'),
                'type' => \Elementor\Controls_Manager::COLOR,
                'default=' => '36731f'
            ]
        );
        ...
        $arg->add_control(
            'thickness',
            [
                'label' => __('Thickness', 'pitoon-addon'),
        ...
        $arg->add_control(
            'linecap',
            [
                'label' => __('Line Cap', 'pitoon-addon'),
                'type' => \Elementor\Controls_Manager::SELECT2,
                'default' => 'square',
                'options' => [
                    'butt' => __('Butt', 'pitoon-addon'),
                    'round' => __('Round', 'pitoon-addon'),
                    'square' => __('Square', 'pitoon-addon'),
                ]
            ]
        );
...
if (!function_exists('pitoon_apply_circle_options')) :
    function pitoon_apply_circle_options($settings, $item)
    {
...
if (!function_exists('pitoon_get_swiper_options')) :
    function pitoon_get_swiper_options($settings, $pagination_id = false, $nav_prev_id = false, $nav_next_id = false)
    {
        $loop_status = ('yes' == $settings['loop']) ? 'true' : 'false';
        $autoplay_status = ('yes' == $settings['autoplay']) ? 'true' : 'false';
        $hover_pause_status = ('yes' == $settings['hover_pause']) ? 'true' : 'false';
...
if (!function_exists('pitoon_get_elementor_carousel_options')) :
    function pitoon_get_elementor_carousel_options($arg, $condition = false)
    {
        ...
        if (!empty($condition)) :
            $arg->start_controls_section(
                'slider_options',
                [
                    'label' => __('Slider Options', 'pitoon-addon'),
                    'tab' => \Elementor\Controls_Manager::TAB_CONTENT,
                    'condition' => [
                        'layout_type' => $condition
                    ]
        ...
        $arg->add_control(
            'autoplay',
            [
                'label' => esc_html__('AutoPlay', 'pitoon-addon'),
                'type' => \Elementor\Controls_Manager::SWITCHER,
        ...
        $arg->add_control(
            'delay',
            [
                'label' => __('AutoPlay Delay', 'pitoon-addon'),
                'type' => \Elementor\Controls_Manager::SLIDER,
                'size_units' => ['count'],
        ...
        $arg->add_control(
            'loop',
            [
                'label' => esc_html__('Loop', 'pitoon-addon'),
                'type' => \Elementor\Controls_Manager::SWITCHER,
                'label_on' => esc_html__('Yes', 'pitoon-addon'),
                'label_off' => esc_html__('No', 'pitoon-addon'),
                'return_value' => 'yes',
        ...
        $arg->add_control(
            'enable_nav',
            [
                'label' => esc_html__('Display Nav', 'pitoon-addon'),
                'type' => \Elementor\Controls_Manager::SWITCHER,
                'label_on' => esc_html__('Yes', 'pitoon-addon'),
                'label_off' => esc_html__('No', 'pitoon-addon'),
                'return_value' => 'yes',
        ...
        $arg->add_control(
            'nav_left_icon',
            [
                'label' => esc_html__('Nav Left Icon', 'pitoon-addon'),
        ...
        $arg->add_control(
            'nav_right_icon',
            [
                'label' => esc_html__('Nav Right Icon', 'pitoon-addon'),
                'type' => \Elementor\Controls_Manager::ICONS,
                'condition' => [
                    'enable_nav' => 'yes'
                ],
        ...
        $arg->add_control(
            'enable_dots',
            [
                'label' => esc_html__('Display Dots', 'pitoon-addon'),
                'type' => \Elementor\Controls_Manager::SWITCHER,
                'label_on' => esc_html__('Yes', 'pitoon-addon'),
                'label_off' => esc_html__('No', 'pitoon-addon'),
                'return_value' => 'yes',
        ...
        $arg->add_control(
            'smart_speed',
            [
                'label' => __('Smart Speed', 'pitoon-addon'),
                'type' => \Elementor\Controls_Manager::SLIDER,
                'size_units' => ['count'],
        ...
        $arg->add_control(
            'items',
            [
                'label' => __('Slide Items', 'pitoon-addon'),
                'type' => \Elementor\Controls_Manager::SLIDER,
                'size_units' => ['count'],
                'range' => [
                    'count' => [
        ...
        $breakpoint->add_control(
            'screen_size',
            [
                'label' => __('Screen Size', 'pitoon-addon'),
                'type' => \Elementor\Controls_Manager::SLIDER,
                'size_units' => ['count'],
                'range' => [
                    'count' => [
        ...
        $breakpoint->add_control(
            'item',
            [
                'label' => __('Slide Item', 'pitoon-addon'),
                'type' => \Elementor\Controls_Manager::SLIDER,
                'size_units' => ['count'],
                'range' => [
                    'count' => [
        ...
        $arg->add_control(
            'breakpoint',
            [
                'label' => __('Breakpoints', 'pitoon-addon'),
                'type' => \Elementor\Controls_Manager::REPEATER,
                'prevent_empty' => false,
...
if (!function_exists('pitoon_elementor_general_style_options')) :
    function pitoon_elementor_general_style_options($agrs, $label, $selector, $condition, $style = 'color', $typo = true, $color = true)
    {
        ...
        if ($typo) :
            $agrs->add_group_control(
                ...
                [
                    ...
                    'label'          => esc_html__(' Typography', 'pitoon-addon'),
                    'selector'       => $selector,
                    'condition' => [
                        'layout_type' => $condition
                    ]
...
if (!function_exists('pitoon_elementor_button_style_options')) :
    function pitoon_elementor_button_style_options($init, $label, $selector, $hover_bg_selector = '', $condition = 'layout_one')
    {
        ...
        $init->add_control(
            ...
            [
                'label' => __('Border Radius', 'pitoon-addon'),
                'type' => \Elementor\Controls_Manager::DIMENSIONS,
                'size_units' => ['px', '%'],
                'selectors' => [
        ...
        $init->add_control(
            str_replace(' ', '_', $label) . '_color',
            [
                'label' => __('Text Color', 'pitoon-addon'),
                'type' => \Elementor\Controls_Manager::COLOR,
                'default' => '',
                'selectors' => [
        ...
        $init->add_control(
            ...
            [
                'label' => __('Background Color', 'pitoon-addon'),
                'type' => \Elementor\Controls_Manager::COLOR,
                'selectors' => [
        ...
        $init->add_control(
            ...
            [
                'label' => __('Border Color', 'pitoon-addon'),
                'type' => \Elementor\Controls_Manager::COLOR,
                'condition' => [
...
if (!function_exists('pitoon_elementor_heading_option')) :
    function pitoon_elementor_heading_option($init, $label, $default = 'h2', $layout = '')
    {
        $init->add_control(
            str_replace(' ', '_', strtolower($label)) . '_tag_' . $layout,
            [
                'label' => esc_html__($label . ' Tag', 'pitoon-addon'),
...
if (!function_exists('pitoon_elementor_rendered_content')) :
    function pitoon_elementor_rendered_content($init, $rendered_name, $class = '', $tag = 'p', $url_name = 'url', $extra = '')
    {
        ...
        if ('a' == $tag) :
            printf(
                ...
                wp_kses($settings[$rendered_name], 'pitoon_allowed_tags'),
                $extra,
...
if (!function_exists('pitoon_elementor_repeater_rendered_content')) :
    function pitoon_elementor_repeater_rendered_content($init, $content, $key, $rendered_name, $class = 'pitoon-default', $tag = 'p', $url_name = 'url', $extra = '')
    {
        if ('a' == $tag) :
        ...
        if ('a' == $tag) :
            printf(
                ...
                wp_kses($content[$rendered_name], 'pitoon_allowed_tags'),
                $extra,
                'class="' . esc_attr($class) . '"'
            );
...
if (!function_exists('pitoon_basic_rendered_content')) :
    function pitoon_basic_rendered_content($init, $content, $rendered_name, $class = 'pitoon-default', $tag = 'p', $url_name = 'url', $extra = '')
    {
...
if (!function_exists('pitoon_elementor_rendered_image')) {
    function pitoon_elementor_rendered_image($content, $name, $class = '', $duration = '', $delay = '')
    {
...
if (!function_exists('pitoon_elementor_column_count_options')) :
    function pitoon_elementor_column_count_options($init, $condition)
    {
        $init->start_controls_section(
            'column_options',
            [
                'label' => __('Column Options', 'pitoon-addon'),
                'tab' => \Elementor\Controls_Manager::TAB_CONTENT,
                'condition' => [
                    'layout_type' => $condition
                ]
        ...
        $init->add_responsive_control(
            'column_count',
            [
                'label' => __('Column Width %', 'pitoon-addon'),
                'type' => \Elementor\Controls_Manager::SLIDER,
                'range' => [
                    '%' => [
                        'min' => 1,
...
if (!is_rtl()) :
    ...
    function pitoon_set_rtl_mode($locale)
    {
        ...
        if (isset($current_page->ID)) {
            $check_page_rtl = get_post_meta($current_page->ID, 'pitoon_enable_rtl_mode', true);
            $pitoon_get_rtl_mode_status = empty($check_page_rtl) ? $pitoon_get_rtl_mode_status : $check_page_rtl;
        }
        ...
        if ($check_url_without_http ==  $site_without_http) {
            ...
            $check_home_page_rtl = get_post_meta($frontpage_id, 'pitoon_enable_rtl_mode', true);
            $pitoon_get_rtl_mode_status = empty($check_home_page_rtl) ? $pitoon_get_rtl_mode_status : $check_home_page_rtl;
        }

        $pitoon_dynamic_rtl_mode_status = isset($_GET['rtl_mode']) ? $_GET['rtl_mode'] : $pitoon_get_rtl_mode_status;
        if ('yes' == $pitoon_dynamic_rtl_mode_status) {
            $locale = ($pitoon_dynamic_rtl_mode_status == 'yes') ? 'ar' : 'en_US';
        }

        return $locale;
    }

    add_filter('locale', 'pitoon_set_rtl_mode', 1, 1);
 ...
 */
if (!function_exists('pitoon_auto_add_product_to_cart')) :
    function pitoon_auto_add_product_to_cart()
    {
        $auto_cart_status = isset($_GET['auto_cart']) ? $_GET['auto_cart'] : '';

        if (!is_admin() && !empty($auto_cart_status)) {
            $get_product = pitoon_get_page_by_title($auto_cart_status, $post_type = "product"); // 64; //replace with your own product id
    ...
    add_action('template_redirect', 'pitoon_auto_add_product_to_cart');
endif;
...
if (!function_exists('pitoon_product_details_social_share')) :
    ...
    function pitoon_product_details_social_share()
    {
        ...
        $pitoon_url = urlencode_deep(get_permalink());
        //get current page title
        $pitoon_title = str_replace(' ', '%20', get_the_title($post->ID));
        //get post thumbnail for pinterest
        $pitoon_thumbnail = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'full');

        //all social share link generate
        $facebook_share_link = 'https://www.facebook.com/sharer/sharer.php?u=' . $pitoon_url;
        $twitter_share_link = 'https://twitter.com/intent/tweet?text=' . $pitoon_title . '&amp;url=' . $pitoon_url . '&amp;via=Crunchify';;
        $linkedin_share_link = 'https://www.linkedin.com/shareArticle?mini=true&url=' . $pitoon_url . '&amp;title=' . $pitoon_title;;
        $pinterest_share_link = 'https://pinterest.com/pin/create/button/?url=' . $pitoon_url . '&amp;media=' . $pitoon_thumbnail[0] . '&amp;description=' . $pitoon_title;

    ?>
        <div class="product-details__social">
            <div class="title">
                <h3><?php esc_html_e('Share with Friends', 'pitoon'); ?></h3>
            </div>
...
if (!function_exists('pitoon_page_header_extra_class_callback')) :
    function pitoon_page_header_extra_class_callback($class)
    {
        return '';
    }
endif;
add_filter('pitoon_page_header_extra_class', 'pitoon_page_header_extra_class_callback', 10, 1);
